...
Public Sub SauvegarderBrouillon(ByVal doc As Document)
    ...
    If Len(reservation) > 0 Then
        If modFichiers.FichierExiste(reservation) Then
...
Public Sub TransmettreSecretariat(ByVal doc As Document, ByVal cheminDocx As String)
    Dim typeCourrier As String, source As String, destination As String, fso As Object
    typeCourrier = modValidation.ValiderDocument(doc, True)
    source = Trim$(VariableDoc(doc, "ReservationNas"))
    If Len(source) > 0 Then
        Set fso = CreateObject("Scripting.FileSystemObject")
        destination = modConfig.Chemin("Echange") & "\Arrives\Pris\" & fso.GetFileName(source)
        If fso.FileExists(source) Then modFichiers.RenommerAtomique source, destination
    End If
End Sub
...
Public Sub Unifie_ReprendreBrouillon()
    ...
    For Each fichier In modFichiers.ListerFichiers(modConfig.Chemin("Echange") & "\Arrives\EnCours", ".txt")
        Set d = modFichiers.LireDrapeau(CStr(fichier))
