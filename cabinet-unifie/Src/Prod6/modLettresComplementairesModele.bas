...
End Function
Private Function LCM_ChoisirModeleParDialogue( _
    ByVal afficherConfirmationAnnulation As Boolean) As String

    Dim dlg As Object
    Dim chemin As String

    LCM_ChoisirModeleParDialogue = ""
    ...
    With dlg
        ...
        .Title = _
            "Sélectionner le modèle Word utilisé pour la dictée"

        .AllowMultiSelect = False

        .Filters.Clear
        .Filters.Add _
            "Modèles / documents Word", _
            "*.dot;*.dotx;*.dotm;*.docx;*.docm"
        ...
        If .Show <> -1 Then
            ...
            If afficherConfirmationAnnulation Then
        ...
        chemin = CStr(.SelectedItems(1))

    End With

    If Not LCM_FichierExiste(chemin) Then

        MsgBox _
            "Le fichier sélectionné est introuvable.", _
            vbExclamation
    ...
    SaveSetting _
        LCM_APP_REGISTRE, _
        LCM_SECTION_REGISTRE, _
        LCM_CLE_REGISTRE, _
        chemin

    LCM_ChoisirModeleParDialogue = chemin
    Exit Function

GestionErreur:

    MsgBox _
        "Impossible de sélectionner le modèle Word." & vbCrLf & _
...
Private Function LCM_CheminModeleCompatible( _
    ByVal chemin As String) As Boolean

    Dim nomFichier As String

    LCM_CheminModeleCompatible = False

    If Not LCM_FichierExiste(chemin) Then Exit Function

    nomFichier = LCase$(Dir$(chemin))

    If InStr(1, nomFichier, _
        "lettremodeledragon", vbTextCompare) > 0 Then

        LCM_CheminModeleCompatible = True

    End If

End Function

Private Function LCM_ChercherDansDossier( _
    ByVal dossier As String) As String

    Dim noms As Variant
    Dim i As Long
    Dim chemin As String

    LCM_ChercherDansDossier = ""

    dossier = Trim$(dossier)

    If dossier = "" Then Exit Function

    If Right$(dossier, 1) <> "\" Then
        dossier = dossier & "\"
    End If

    noms = Array( _
        "lettremodeledragon.dotm", _
        "lettremodeledragon.docx", _
        "lettremodeledragon.docm", _
        "lettremodeledragon(1).dotm", _
        "lettremodeledragon.dotx", _
        "lettremodeledragon.dot")

    For i = LBound(noms) To UBound(noms)

        chemin = dossier & CStr(noms(i))

        If LCM_FichierExiste(chemin) Then
            LCM_ChercherDansDossier = chemin
            Exit Function
        End If
...
Private Function LCM_FichierExiste( _
    ByVal chemin As String) As Boolean

    On Error Resume Next

    LCM_FichierExiste = _
        (Trim$(chemin) <> "" And Len(Dir$(chemin)) > 0)
